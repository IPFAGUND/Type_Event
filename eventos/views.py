...
#from io import BytesIO  serve para salvar a imagem temporariamente numa variavel
from django.core.files.uploadedfile import InMemoryUploadedFile
from PIL import Image, ImageDraw, ImageFont
import sys
#importacoes devem ficar listadas por ordem alfabetica
...
def inscrever_evento(request, id):
    ...
    if request.method == "GET":
        ...
        #TODO:  Validar se o usuario ja é um participante (Validar login)
...
def participantes_evento(request, id):
    ...
    if not evento.criador == request.user:
        ...
        #verifica se quem esta acessando é realmente proprietario do evento
    if request.method == "GET":
...
def gerar_certificado(request, id):
    ...
    for participante in evento.participantes.all():
        ...
        output = BytesIO()
        #BytesIO  serve para salvar a imagem temporariamente numa variavel
        img.save(output, format="PNG", quality=100)
        output.seek(0)
        #retorna posicao da memoria para o inicio, assim podemos ler
        img_final = InMemoryUploadedFile(output,
                                        ...
                                        None)
        #InMemoryUploadedFile serve apenas para arquivos pequenos.
        #TemporaryUploadFile deve ser usado para arquivos maiores.
        #token_urlsafe gera um conjunto de caracteres aleatorio para salvar o arquivo com nome unico.
        #sys.getsizeof(output) verifica o tamanho do arquivo.
...
def procurar_certificado(request, id):
    ...
    #esse underline duplo de participante__email é para buscar um valor em outra tabela relacionada
    if not certificado:
